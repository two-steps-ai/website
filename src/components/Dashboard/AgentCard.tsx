...
const AgentCard: React.FC<AgentCardProps> = ({
  ...
  const borderClass = isActive ? 'border-blue-500/50' : 'hover:border-blue-500/30';

  return (
    <motion.div whileHover={{ scale: 1.02 }} className="relative group">
      <div
        className={clsx(
          'relative overflow-hidden rounded-2xl border transition-all duration-300 bg-gray-900/50 backdrop-blur-xl border-gray-800/50',
          borderClass
        )}
      >
        <div className="p-6">
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-2">
              <span className="flex items-center gap-1.5">
                <Activity
                  className={clsx('w-4 h-4', status === 'Active' ? 'text-green-400' : 'text-red-400')}
                />
                <span className={clsx('text-sm', status === 'Active' ? 'text-green-400' : 'text-red-400')}>
                  {status}
                </span>
              </span>
            </div>
          ...
          <div className="grid grid-cols-2 gap-3 mb-4">
            {Object.entries(metrics).map(([key, value]) => (
              <div key={key} className="bg-gray-900/50 rounded-xl p-3 backdrop-blur-sm">
                ...
                <p className={clsx('font-semibold bg-gradient-to-r', gradient, 'bg-clip-text text-transparent')}>
                  {value}
                </p>
              </div>
            ))}
