...
interface TabletCaseStudyModalProps {
  study: CaseStudy | null;
  onClose: () => void;
  onBookCall: () => void;
}

const TabletCaseStudyModal: React.FC<TabletCaseStudyModalProps> = ({
  study,
  onClose,
  onBookCall
}) => {
            ...
            {study.implementation.map((step, index) => (
              ...
              >
                <div className="bg-gray-900/50 rounded-xl p-4 border border-gray-800/50">
                  <h5 className={`text-sm sm:text-base font-semibold bg-gradient-to-r ${study.gradient} bg-clip-text text-transparent mb-2`}>
                    {index + 1}. {step.title}
                  </h5>
...
export default TabletCaseStudyModal;