...
const CaseStudyModal: React.FC<CaseStudyModalProps> = memo(({ study, onClose, onBookCall }) => {
  const {
    title,
    gradient,
    client,
      ...
      <div className="flex flex-col h-[calc(100dvh-65px)] sm:h-[min(600px,80vh)]">
            ...
            {implementation.map((step, index) => (
              ...
              >
                <div className="bg-gray-900/50 rounded-xl p-4 border border-gray-800/50">
                  <h5
                    className={`text-sm sm:text-base font-semibold bg-gradient-to-r ${gradient} bg-clip-text text-transparent mb-2`}
                  >
                    {index + 1}. {step.title}
                  </h5>
                  <ul className="space-y-2">
                    {step.details.map((detail, detailIndex) => (
                      <li key={detailIndex} className="flex items-start gap-2">
                        <div
                          className={`w-1.5 h-1.5 rounded-full bg-gradient-to-r ${gradient} mt-1.5 shrink-0`}
                        />
        ...
        <div className="sticky bottom-0 p-4 sm:p-6 bg-gradient-to-t from-black via-black/95 to-transparent">
          <motion.button
            ...
            whileTap={{ scale: 0.98 }}
            className={`w-full bg-gradient-to-r ${gradient} px-6 py-3 rounded-xl text-sm sm:text-base text-white font-medium flex items-center justify-center gap-2 hover:shadow-lg transition-all duration-300 touch-manipulation`}
          >
            Book a Call
