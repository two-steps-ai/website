...
const CaseStudyCard: React.FC<CaseStudyCardProps> = ({ study, index, onSelect }) => {
  ...
  return (
    <motion.div
      initial={cardAnimationVariants.initial}
      animate={cardAnimationVariants.animate}
      transition={{ duration: 0.5, delay: index * 0.1 }}
      className="group relative"
    >
      <div
        className={clsx(
          'relative overflow-hidden rounded-3xl bg-gray-900/50 backdrop-blur-xl border border-gray-800/50',
        ...
        <div className={clsx('grid grid-cols-1 lg:grid-cols-2 gap-8 p-8')}>
          <div className="space-y-6">
            <div className="flex items-start gap-4">
              <div
                className={clsx(
                  'w-14 h-14 rounded-xl p-[1px] shrink-0',
            ...
            <div className="space-y-4">
              ...
              >
                <span className={clsx(`bg-gradient-to-r ${gradient} bg-clip-text text-transparent`)}>
                  Learn More
                </span>
                <ArrowRight
                  className={clsx(
      ...
      <div
        className={clsx(
          'absolute -inset-2',
          `bg-gradient-to-r ${gradient}`,
          'rounded-[2rem] opacity-0 group-hover:opacity-10 blur-xl transition-opacity duration-500 -z-10'
        )}
      />
    </motion.div>
  );
